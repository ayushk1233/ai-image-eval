...
def render_criteria_radar_chart(leaderboard_data):
    ...
    criteria = ['avg_overall', 'avg_prompt_adherence', 'avg_visual_quality', 'avg_indian_relevance']
    clean_labels = ['Overall', 'Prompt Adherence', 'Visual Quality', 'Indian Relevance']
    ...
    pivot_df.plot(kind='bar', ax=ax, color=['#FF6B6B', '#4ECDC4', '#FFE66D'])
    
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['bottom'].set_color('#FAFAFA')
    ax.spines['left'].set_color('#FAFAFA')
    ax.tick_params(colors='#FAFAFA', rotation=0)
    
    ax.set_ylabel('Average Score (1-5)', color='#FAFAFA', fontsize=12)
    ax.legend(facecolor='#0E1117', labelcolor='#FAFAFA')
    ax.set_ylim(1, 5)
